    ...
    public class UserProfileService : ICrud<UserProfile>
    {
            ...
            try
            {
                ...
                foreach (var row in worksheet.Rows)
                {
                    if (counter > 0)
                    {
                        ...
                        newUserProfile.Username = string.IsNullOrEmpty(row.Cells[10].Value?.ToString().Trim()) ? MailHelper.GenerateEmailFromName(newUserProfile.FullName.Trim(), "MoslemToolkit.online") : row.Cells[10].Value?.ToString().Trim();
        ...
        public byte[] ExportToExcel()
        {
            ...
            foreach (var item in datas)
            {
                ...
                worksheet.Cells[row, 8].Value = item.Role == Roles.Pengurus ? "Ya" : "Tidak";
